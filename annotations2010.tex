...
In this paper I present a method of simultaneously reducing the sizes of these lists and the amount of redundant component code using source code annotations.
I use these annotations as a basis for an ontology that in turn helps reduce the number of components one must be familiar with to create a system configuration.
This helps reduce the cognitive burden of using the system to a reasonable level.
I additionally employ these annotations to eliminate a large amount of redundant, uncoordinated code and enforce a link between components' behavior and semantic metadata.
...
In this paper I demonstrate a method of generating semantic descriptions from source code annotations and then using these semantic descriptions to factor out significant quantities of duplicated and uncoordinated source code.
...
For example, figure \ref{fig:acolyte} depicts the evaluation chain that sets sales quotas and prices in a relatively simple MinneTAC workflow.

\begin{figure}[ht]
\centering
\includegraphics[width=\textwidth]{figures/acolyte}
\caption{\label{fig:acolyte}Evaluator chain for sales quota and pricing}
\end{figure}
...
However, this method comes at the cost of static type checking.
Systems built this way provide no guarantees that components implement the interfaces their annotations claim to support, causing errors stemming from this to fail to appear until runtime.
...
MinneTAC can benefit from a simpler, ``pipe-and-filter'' architecture that makes system design using semantics easier.
It can also benefit from access to component source code by employing methods that use Java annotations, which help link component code with their semantic descriptions.
...
I discuss my solution to this problem in section \ref{sec:approach}, where I describe an approach to applying semantic descriptions to the MinneTAC agent.

\section{Approach}
\label{sec:approach}

I aim to simplify MinneTAC's configuration process by creating a graphical editor for agent workflow configurations that draws upon ideas from Kim et al's Composition Analysis Tool~\cite{kim2004intelligent} as well as those from the systems surveyed in section \ref{sec:challenges}.
...
Systematically generating a list of all of the valid choices throughout the workflow building process lets one take context into account and only display Evaluators whose input or output data types are compatible with a given Evaluator.
...
I accomplish this by creating a knowledge base of Evaluators and their input and output parameters using Web Ontology Language Description Logic (OWL-DL)\footnote{http://www.w3.org/TR/owl2-overview/}.
...
I take advantage of this domain knowledge by placing only these primitive types in the domain term ontology and letting component ontologies define composite types on their own using templates for composite types that are also specified in the domain term ontology.
Simple OWL rules determine whether composite types specified by different component ontologies are compatible or not.
Employing knowledge of the domain in this way solves a number of problems that typically plague workflow composition systems:
...
Where the proper balance between larger, more specific domain term ontologies and smaller, more understandable domain term ontologies lies depends heavily on the domain in question.

\subsection{Using domain type information}

To fill out the component ontology one first needs to define an ontological class for each Evaluator.
...
Listing \ref{lst:input-composite} is an example of choosing a simple type over a specific type.
Referring to product quotas as Quantities rather than ProductQuotas reduces the size of the domain term ontology.
...
Applying the same procedure to Evaluator configuration parameters helps reduce the amount of boilerplate code even further.
...
Constraining Evaluators to using only types given by their semantic descriptions eliminates one of the most serious problems with systems that use semantics:  having component code that does not match its semantic description.
...
In this paper I employ code annotations to specify semantic metadata for MinneTAC's Evaluators, which allows us to generate ontological classes for each Evaluator and query these ontologies to substantially reduce the number of Evaluators an agent workflow designer must be familiar with at once.
...
I additionally use these annotations to eliminate a large amount of redundant code in each component in a way that helps enforce coherence between annotations and component logic.
Though I have not seen all of the methods employed to do so used at once, I expect the use of code annotations for attribute-oriented programming to become more common in the future.

One major limitation of my current annotation-based design is that it supports only one output type per Evaluator.
...
It should be possible to simplify this process by specifying array cell types and index types as separate attributes to Input and Output annotations.

\section*{Acknowledgements}

I would like to thank advisor John Collins and Ali Abu-El-Haj for their work on the MinneTAC ontology, as well as John Collins, Wolfgang Ketter, and Maria Gini~\cite{Collins08ECRA} for figures \ref{fig:arch}, \ref{fig:oracle-classes}, and \ref{fig:acolyte}.
